...
plot_sleep_hist_panel <- function(df,
  ...
  stopifnot(all(c("duration","score") %in% names(df)))
  ...
  df2 <- df %>%
    ...
    { if ("non_wear" %in% names(.)) filter(., is.na(non_wear) | non_wear <= non_wear_max) else . }
  
  p_dur <- df2 %>%
    filter(!is.na(duration_h)) %>%
    ggplot(aes(x = duration_h)) +
    geom_histogram(binwidth = binwidth_duration, boundary = 0, closed = "left") +
    facet_wrap(vars(!!sym(by)), scales = if (free_y) "free_y" else "fixed", drop = TRUE) +
    labs(title = paste(phase_label, "- Unen kesto (histogrammi)"),
  ...
  p_sco <- df2 %>%
    filter(!is.na(score)) %>%
    ggplot(aes(x = score)) +
    geom_histogram(binwidth = binwidth_score, boundary = 0, closed = "left") +
    facet_wrap(vars(!!sym(by)), scales = if (free_y) "free_y" else "fixed", drop = TRUE) +
    labs(title = paste(phase_label, "- Oura score (histogrammi)"),
         subtitle = paste("Ryhmittäin:", by),
         x = "Oura score (0–100)", y = "Lukumäärä") +
    theme_minimal(base_size = 12)
  
  p_dur / p_sco + plot_layout(heights = c(1, 1))
}
